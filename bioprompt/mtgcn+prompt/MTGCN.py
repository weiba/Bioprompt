...
dataz = torch.load("/home/yuantao/code/MTGCN-master/data/str_fearures_ori.pkl")
# dataz1 = torch.load("/home/yuantao/code/MTGCN-master/data/str_fearures_ori.pkl").to(device)
# dataz2 = torch.load("/home/yuantao/code/ollama/cancerdata_pt/pan-cancer-16-pre-newmask.pt").to(device)
# dataz = torch.cat((dataz1,dataz2),1)
# dataz = torch.load("/home/yuantao/code/ollama/cancerdata_pt/pan-cancer-16-pre-newmask.pt").to(device)
...
for i in range(10):
    ...
    indices_all = np.where(mask_all)[0]
    print(len(indices_all))
    kf = KFold(n_splits=5, shuffle=True)

    for cv_run, (train_index, test_index) in enumerate(kf.split(indices_all)):
        print("----------------------- i: %d, cv_run: %d -------------------------" % (i + 1, cv_run + 1))
        tr_mask,te_mask = np.zeros(len(mask_all), dtype=bool), np.zeros(len(mask_all), dtype=bool)
        tr_mask[train_index], te_mask[test_index] = True, True
        print(tr_mask.sum(), te_mask.sum())
        ...
        for epoch in range(1, EPOCH):
            train(tr_mask)
            if epoch % 100 == 0:
                print('i:', i, 'cv_run:', cv_run, 'epoch:', epoch)

        AUC[i][cv_run], AUPR[i][cv_run] = test(te_mask)
        print(AUC[i][cv_run], AUPR[i][cv_run])
        np.savetxt("result/MTGCN-AUC-pan-cancer-16-pre.txt", AUC, delimiter="\t")
        np.savetxt("result/MTGCN-AUPR-pan-cancer-16-pre.txt", AUPR, delimiter="\t")
    file = open("result.txt", 'a')
    file.write( "MTGCN-16-pre-newmask\t" + "AUC: "+str(AUC.mean()) + "\t" + "AUPR: "+str(AUPR.mean()) + "\n")
