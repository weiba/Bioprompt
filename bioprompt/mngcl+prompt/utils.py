...
def Config(dataset='CPDB', cancerType='pan-cancer'):
    parser = argparse.ArgumentParser()
    parser.add_argument('--dataset', type=str, default=dataset)
    parser.add_argument('--cancer_type', type=str, default=cancerType)
    parser.add_argument('--device', type=int, default=0)
    parser.add_argument('--config', type=str, default='/home/yuantao/code/MNGCL-master/config.yaml')
    args = parser.parse_args()
    config = yaml.load(open(args.config), Loader=SafeLoader)[args.dataset]
    dataPath = "/home/yuantao/code/MNGCL-master/data/" + args.dataset + "/"
...
def dataLoader(args,dataPath,cancerType,device):
    ...
    if cancerType == 'pan-cancer':
        ...
        data.bert_output = torch.load("/home/yuantao/code/LLM/bert_out/PAN-CANCER-weight.pt").to(device)
        ...
        data.bert_output = torch.load("/home/yuantao/code/LLM/bert_out/" + cancerType.upper() + "-weight.pt").to(device)
